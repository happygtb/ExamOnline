 ...
 */
	...
	$.getJSON("../QuestionJson",{pageNow:pageNow,Qtype:Qtype}, function(jsonData) {
		for (var i = 0; i < jsonData.length; i++) {
			var question= "<div class=\"items\" id=\"items"+(i+1)+"\"><li><span>"+(i+1)+".<label class=\"type\" name=\""+jsonData[i].TypeName+"\" id=\"TypeName"+i+"\">["+jsonData[i].TypeName+"]</label><label class=\"tips\" id=\"tips"+i+"\">"+jsonData[i].SubjectName+"-"+jsonData[i].TipsName+"</label><label class=\"Difficulty\" id=\"Difficulty"+i+"\">"+jsonData[i].Difficulty+"星</label></span>";
	...
	$.getJSON("../PageJson",{Qtype:Qtype}, function(jsonData) {			
		for (var i = 0; i < jsonData.length; i++) {
			if(pageNow!="1"){
				pages+="<li><a href='QuestionIndex.html?pageNow="+(s_pageNow-1)+"&Qtype="+Qtype+"' aria-label='Previous'><span aria-hidden='true'>&laquo;</span></a></li>";
			}else{
			...
			var pageCount=parseInt(s_pageCount);
 			for(var j=1;j<=pageCount;j++){
 				...
 				pages+="<li id='page"+j+"'><a href='QuestionIndex.html?pageNow="+j+"&Qtype="+Qtype+"'>"+j+"</a></li>";
 			}
   			if(pageNow!=s_pageCount){
   	 			pages+="<li><a href='QuestionIndex.html?pageNow="+(s_pageNow+1)+"&Qtype="+Qtype+"' aria-label='Next'><span aria-hidden='true'>&raquo;</span></a></li>";
   			}else{
		...
		$.getJSON("../OptionJson?QuestionId="+id, function(jsonData) {
			...
			for (var i = 0; i < jsonData.length; i++) {
				...
				if(type=="单选题"){
					...
					option += "<p><input type='radio' name=\""+id+(i+1)+"\" checked='checked'>"+jsonData[i].OptionDetails+"</p>";
					}else{
					option += "<p><input type='radio' name=\""+id+(i+1)+"\">"+jsonData[i].OptionDetails+"</p>";
					} 
				}
				else if(type=="多选题"){
					if(jsonData[i].IsCorrect=="True"){
					option += "<p><input type='checkbox' name=\""+id+(i+1)+"\" checked='checked'>"+jsonData[i].OptionDetails+"</p>";
					}else{
					option += "<p><input type='checkbox' name=\""+id+(i+1)+"\">"+jsonData[i].OptionDetails+"</p>";
					} 
				}
				else if(type=="判断题"){
					if(jsonData[i].IsCorrect=="True"){
						option +="<p><input type='radio' name=\""+id+(i+1)+"\" checked='checked'/>正确</p>";
						option +="<p><input type='radio' name=\""+id+(i+1)+"\"/>错误</p>";
					}else{
						option +="<p><input type='radio' name=\""+id+(i+1)+"\"/>正确</p>";
						option +="<p><input type='radio' name=\""+id+(i+1)+"\" checked='checked'/>错误</p>";
					}
				}
				else{
	...
	function Search(){
		...
		$.getJSON("../SearchJson",{Qtype:Qtype,searchcontent:content,page:page}, function(jsonData) {			
			for (var i = 0; i < jsonData.length; i++) {
				 question= "<div class=\"items\" id=\"items"+(i+1)+"\"><li><span>"+(i+1)+".<label class=\"type\" name=\""+jsonData[i].TypeName+"\" id=\"TypeName"+i+"\">["+jsonData[i].TypeName+"]</label><label class=\"tips\" id=\"tips"+i+"\">"+jsonData[i].SubjectName+"-"+jsonData[i].TipsName+"</label><label class=\"Difficulty\" id=\"Difficulty"+i+"\">"+jsonData[i].Difficulty+"星</label></span>";
				 question+="<label onclick='addoptions(this.id)'  id=\""+i+"\">展开V</label><label  name=\""+jsonData[i].QuestionID+"\" id=\"addoptions"+i+"\"></label></li>";
				 question+= "<span class='title'>"+jsonData[i].QuestionDetails+"</span><div class=\"options\" id=\"option"+i+"\" ></div>";
				 if(jsonData[i].Labels!="null"){
